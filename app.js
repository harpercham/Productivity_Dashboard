(() => {
  ...
  const els = {
    ...
    resetFiltersBtn: $("#resetFiltersBtn"), exportBtn: $("#exportBtn"), rigMetric: $("#rigMetric"), stMetric: $("#stMetric"), toast: $("#toast"),
  ...
  function fillSelect(select, values) {
    ...
    select.innerHTML = '<option value="">All</option>' + values.sort(naturalCompare).map((v) => `<option value="${escapeHtml(v)}">${escapeHtml(v)}</option>`).join("");
  ...
  function resetFilters() {
    els.subconFilter.value = els.rigFilter.value = els.stFilter.value = els.typeFilter.value = "";
  ...
  function renderTrendChart() {
    const data = aggregateBy(state.filtered, "dateKey").sort((a, b) => a.name.localeCompare(b.name));
    ...
    state.charts.trend = new Chart($("#trendChart"), {
      ...
      options: { maintainAspectRatio: false, interaction: { mode: "index", intersect: false }, plugins: { legend: { position: "bottom" } },
        scales: { x: { grid: { display: false } }, points: { beginAtZero: true, position: "left", title: { display: true, text: "Points" } }, volume: { beginAtZero: true, position: "right", title: { display: true, text: "Net volume (m³)" }, grid: { drawOnChartArea: false } } } }
    });
  }
